...
First, we need to import our dataset. The dataset consists of a dataframe containing a list of genes and phenotypic traits. These genes and traits have been measured in leaf segments at four different developmental stages and across three closely related species. Additionally, we require an extra dataframe providing useful information about each feature. This information might include details such as the feature type (gene or phenotypic trait), the biological processes in which the feature is involved, the gene family to which the feature belongs, and more.
...
figure <- ggplot(mappings[["neurons_sizes"]], aes(x = Neuron, y = size, fill = species)) +
...
To gain insights into the nature of evolutionary displacements, we employ the net_edges() function to generate a displacement graph. This function constructs a dataframe that counts the number of displacements between neurons and species pairs
...
You can use the plot_displacements() function to observe the extent of feature displacements between species. With this visualization, you can gain insights into the extent and quality of feature displacements between species.
...
### 5.2 Displacements of a specific group of features
...
selected_features <- mappings[["classes"]][mappings[["classes"]]$Feature %in% photo.genes,]
...
net_edges_pk_photo <- net_edges(mappings_selected_features, 
                                     "C3", "PK",
                                     add_disp_type = TRUE,
                                     disp_type = displacements)
head(net_edges_pk_photo, 10)

plot_displacements(mappings = mappings_selected_features,
...
ibs_displaced_bv <- selected_features[selected_features$C4 %in% 6,1]
ibs_displaced_bv
